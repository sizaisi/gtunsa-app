    ...
    <div>
        <b-card class="principal-card" title="Estado de su trámite:">
            ...
            <b-container>
                ...
                <div
                    v-for="(grado_proc_mov, index) in mov_grado_procedimientos"
                    :key="index"
                >
                    <b-row>
                        <b-col
                            v-if="
                                grado_proc_mov.idgradproc_origen ==
                        ...
                        <b-col
                            v-else-if="mostrar_actual_en_movimiento == false"
                        >
                            <estado-component
                                :title="grado_proc_mov.nombre_procedimiento"
                                :number="grado_proc_mov.nro_orden"
                        ...
                        <b-col
                            v-else-if="
                                grado_proc_mov.nro_orden <
                                    orden_actual_en_movimiento ||
                                    grado_proc_mov.idgradproc_origen ==
                ...
                <b-row
                    v-if="
                        grado_proc_actual != null &&
                            mostrar_actual_en_movimiento == false &&
                ...
                <b-collapse v-model="showRest">
                    <div
                        v-for="(grado_proc_rest,
                        index) in resto_grado_procedimientos"
                        :key="index"
                    >
                        <b-row>
                            <b-col>
                                <estado-component
                                    ...
                                    "
                                    :number="grado_proc_rest.nro_orden"
                                    color="gray"
                                    :hideDescription="true"
                                    :tail="true"
                                />
...
export default {
    ...
    methods: {
        getGradoProcedimientoActual() {
            ...
            axios
                .get(`${this.api_url}/grado_procedimiento/actual`, {
                    params: {
                        idgrado_procedimiento_actual: this
                            .idgrado_procedimiento_actual
                    }
                })
                .then(response => {
        ...
        getMovimientos() {
            ...
            axios
                ...
                .then(response => {
                    ...
                    this.calcularEstadosConMovimientos();
                })
                .catch(error => {
                    console.log(error);
                });
        ...
        calcularEstadosConMovimientos() {
            for (let i = 0; i < this.movimientos.length; i++) {
                ...
                var index = this.mov_grado_procedimientos.findIndex(
                    item =>
                        item.idgradproc_origen ===
                        objMovimiento.idgradproc_origen
                );

                if (index == -1) {
                    //si no se encuentra en procedimientos a mostrar agregarlo
                    this.mov_grado_procedimientos.push(objMovimiento);
                } else {
                    // sobreescribir el ultimo movimiento con el procedimiento que fue devuelto
            ...
            for (
                cont = 0;
                cont < this.mov_grado_procedimientos.length;
                cont++
            ) {
                ...
                if (
                    objGradMovimiento.idgradproc_origen ==
                    this.idgrado_procedimiento_actual
                ) {
                    this.mostrar_actual_en_movimiento = true;
                    this.orden_actual_en_movimiento =
                        objGradMovimiento.nro_orden;
                }
            }

            if (this.mostrar_actual_en_movimiento == false) {
                //actual no se encuentra en movimientos como origen
        ...
        getProcedimientosRestantes(idGradProcMaxOrdenMostrado) {
            axios
                .get(`${this.api_url}/grado_procedimiento/resto`, {
                    params: {
                ...
                .then(response => {
                    ...
                    for (
                        let k = 0;
                        k < this.resto_grado_procedimientos.length;
                        k++
                    ) {
                        var objRestoGradoProcedimiento = this
                            .resto_grado_procedimientos[k];
                ...
                if (this.movimientos.length > 0) {
                    if (this.mostrar_actual_en_movimiento == true) {
                        ...
                        for (let k = 0; k < this.movimientos.length; k++) {
                            if (this.movimientos[k].nro_orden > max_orden) {
                                //si es mayor actualizar max orden
                                max_orden = this.movimientos[k].nro_orden;
                                idGradProcOrigen_Max_Orden = this.movimientos[k]
                                    .idgradproc_origen;
                            }
                        }

                        this.getProcedimientosRestantes(
                            idGradProcOrigen_Max_Orden
                        );
                    } else {
                        this.getProcedimientosRestantes(
                            this.idgrado_procedimiento_actual
